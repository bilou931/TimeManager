...
export default function Settings() {
  ...
  const [list_shops, setDisplayShops] = useState([])
  const [boolshop,setBoolTodisplayShop]= useState(false)
  ...
  async function getGpsCoordReverse(coord) {
    let response = await fetch("http://api.positionstack.com/v1/reverse?access_key=ce0cabdffceba88f697570ca071956b7&query=" + coord)
    let content = await response.json()
    let result = content.data[0].name + ", " + content.data[0].county+ ", " + content.data[0].postal_code
    return result
  }

   async function searchShopNearUser(index) {
    console.log(index)
    // let coord = await getGpsCoord(index)
    // let coordonate = coord.replace(",", "%2C")
    // console.log(coordonate)
    // let response = await fetch("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+coordonate+"&radius=1500&type=gas_station&key=AIzaSyAdVJx9w43J2pzxeSXkOh4RveKiEwxNXNQ")
    // let content = await response.json()
    // let result = []
    // result.push("\n"+content.results[0].name+": "+content.results[0].vicinity+ "\n-----------------\n\n"+ content.results[1].name+": "+content.results[1].vicinity + "\n-----------------\n\n"+ content.results[2].name+": "+content.results[2].vicinity)
    // console.log(result)
    // let response2 = await fetch("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+coordonate+"&radius=1500&type=restaurant&key=AIzaSyAdVJx9w43J2pzxeSXkOh4RveKiEwxNXNQ")
    // let content2 = await response2.json()
    // let result_food = []
    // result_food.push("\n"+content2.results[0].name+": "+content2.results[0].vicinity+"\n-----------------\n\n"+ content2.results[1].name+": "+content2.results[1].vicinity + "\n-----------------\n\n"+ content2.results[2].name+": "+content2.results[2].vicinity)
    // console.log(result_food)
    let result = "11 avenue de Verdun, 93150,Chatelet Les Halles,"+"\n-----------------\n\n"+"11 Rue de Bellechasse, 75007 Paris,"+"contentezlfzeop dazod ozadzasdjza d oizda"
    let result_food = "efze avenue de Verdun, 93150,Chatelet Les Halles,"+"\n-----------------\n\n"+"11 Rue de Bellechasse, 75007 Paris,"+"contentezlfzeop dazod ozadzasdjza d oizda"
    return {gas: result, food: result_food}
  }

  async function displayShopNearUser(index) {
    let list = await searchShopNearUser(index)

    setDisplayShops(<View style={styles.divResult}>
    {/* <ImageBackground source={require('../blurBackground.png')} blurRadius={90} style={styles.divResult}> */}
       <Button title="Quit" onPress={()=>setDisplayShops([])}/>
       <Text style={styles.SetTextButton}>Nearest Shop:{"\n"+index}</Text>
       <ScrollView style={styles.divForShops} horizontal={true}>
        <Text style={{fontWeight:"300"}}>Gas Station:{"\n\n"+list.gas}</Text>
       </ScrollView>
       <ScrollView style={styles.divForShops} horizontal={true}>
        <Text style={{fontWeight:"300"}}>Restaurant:{"\n\n"+list.food}</Text>
       </ScrollView>
       {/* </ImageBackground> */}
       </View>
       )
 ...
 return {h: hours, m: minutes, s: second}
  ...
  async function RequestMaps(index) {
    ...
    if (response.status === 200) {
      ...
      for (var i = 0; i != modulable_start_event.length; i++) {
        ...
        if (new Date(starting_time[(starting_time.length - 1)]).getTime() < new Date(modulable_start_event[i]).getTime() && new Date(modulable_start_event[i]).getTime() < new Date(start_event[index]).getTime()
          ...
          if ( insert.bool === true) {
            ...
            temp_path.push(modulable_location_event[i])
            let copie = modulable_location_event[i]
            buttonlist.push(<Button color={'#800080'} title="See shops around you" onPress={ ()=> {
              setBoolTodisplayShop(true)
              // if (boolshop === true) {
              displayShopNearUser(copie)
              // }
            }}/>)
            ...
            let copie2 = location_event[index]
            buttonlist.push(<Button color={'#800080'} title="See shops around you" onPress={ ()=> { 
              setBoolTodisplayShop(true)
              // if (boolshop === true) {
                displayShopNearUser(copie2)
              // }
            }}/>)
            // insert event
          }
      ...
      temp_path.push(location_event[index])
      let copie3 = location_event[index]
      buttonlist.push(<Button color={'#800080'} title="See shops around you" onPress={ ()=> { 
        setBoolTodisplayShop(true)
        // if (boolshop === true) {
        displayShopNearUser(copie3)
        // }
      }}/>)
      console.log("temp path =====  " + temp_path)
    ...
    if (response.status === 200) {
      if (content.items.length === 0) {
        ...
        buttonlist.push(<Button color={'#800080'} title="See shops around you" onPress={ ()=> { 
          setBoolTodisplayShop(true)
          // if (boolshop === true) {
          displayShopNearUser(startadressrequest)
          // }
        }}/>)
  ...
  function display_result() {
    ...
    for (var i = 0; i < temp_path.length; i++) {
      if (i === 0) {
        temp.push(<View key={i} style={styles.divForResult1}>
        ...
        temp.push(<View key={i} style={styles.divForResult}>
          ...
          {/* <Button title="See shops around you" onPress={ ()=>  searchShopNearUser(i-1)}/> */}
    ...
    <StatusBar hidden />
      ...
      <ScrollView  contentContainerStyle={{ flexGrow: 1 , alignItems:"center"}} style={styles.scroll}>
        <View style={styles.DivForEachData}>
          ...
          <View style={{ backgroundColor: "black", width: "45%" ,borderRadius:15 }}>
            <Button title="Search" onPress={() => getCalendarEvent()} />
          </View>
        </View>
